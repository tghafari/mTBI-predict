function [resp,t,waitStart]=runGrating(cfg,condMat,startTime,time,sendTrigger,picTex,recPos,recPos_other,act_bl_num,vbl,curr_bl_prac,eylnkMsg,logMsg,LPTtrig)
...
while 1
    ...
    elseif t >condMat(i,j,6) && t <cfg.endTime
     ...
     if t < othswtime1
         picnum_other=picnum;
    elseif t >= othswtime1 && t< othswtime2
        picnum_other=1+mod((picnum_other-1)+cfg.initspeed+dsp1,300);
    elseif t >= othswtime2 
        picnum_other=1+mod((picnum_other-1)+cfg.initspeed+dsp2,300);
     end
